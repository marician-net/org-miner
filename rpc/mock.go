...
func (c *mockClient) FilterLogs(ctx context.Context, query ethereum.FilterQuery) ([]types.Log, error) {
	tokenAbi, _ := abi.JSON(strings.NewReader(contracts1.ZapLibraryABI))
	ev := tokenAbi.Events["NonceSubmitted"]
	event1 := contracts1.ZapLibraryNonceSubmitted{
	...
	//eventResult := contracts1.ZapLibraryNonceSubmitted{}
