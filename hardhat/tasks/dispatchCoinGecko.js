...
task("dispatchCoinGecko", "creates an offchain Incoming event")
    ...
    .setAction(async () => {
        ...
        // console.log("### Endpoint: " + specifier)
        const subscriberAccount=signers[2];

        // console.log("### owner: " + owner.address + " subscriberAccount: " + subscriberAccount.address);
        ...
        console.log("############################")
        await bondage.connect(subscriberAccount)
            .bond(owner.address, specifier, 100);
        let res=await subscriber
            .connect(subscriberAccount)
            .testQuery(owner.address, query, specifier, params);
        // let res = await dispatch.connect(subscriberAccount).query(owner.address, query, specifier, params);
